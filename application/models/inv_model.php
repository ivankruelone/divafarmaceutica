...
on duplicate key update caducidad=values(caducidad), inv = cast(values(inv) + inv as signed), modificado = now(), tipo = values(tipo), subtipo = values(subtipo), idref = values(idref);";
    ...
    function get_entradas($subtipo, $estatus = 0)
    {
        ...
        $this->db->join('movimientos m', 'c.tipo = m.tipo and c.subtipo = m.subtipo',
            'LEFT');
    ...
    function get_productos_entrada_rechazados($id, $orden = 'i.id', $direccion =
        'ASC')
    {
        $this->db->select('i.*, clave, descripcion');
    ...
    function submit_captura_clave_entrada()
    {
        ...
        if ($lc == 0) {
            ...
            if (caduca_en_dias($caducidad) > $this->caducidad_minima || $r2->tipo == 2 || $r2->
                tipo == 4 || $caducidad == '0000-00-00') {
...
(SELECT p_id, case when p.lc = 0 then 'SL' else lote end as lote, case when p.lc = 0 then '0000-00-00' else caducidad end as caducidad, d.piezas, now(), tipo, subtipo, d.id, i.sucursal_id, i.proveedor_id, " .
...
on duplicate key update caducidad=values(caducidad), inv = cast((ifnull(inv, 0) - values(inv)) as signed), modificado = now(), tipo = values(tipo), subtipo = values(subtipo), idref = values(idref), sucursal_id = values(sucursal_id), proveedor_id = values(proveedor_id), user_id = values(user_id);";
    ...
    function cancela_folio($id, $motivo)
    {
        ...
        $data = array('estatus' => 2, 'motivo' => strtoupper($motivo));
    ...
    function reporte_header($id)
    {
        ...
        $logo = array('src' => base_url() . 'images/logo.png', 'width' => '120');
        ...
        $tabla = '<table cellpadding="5">
            <tr>
                ...
                <td rowspan="3" width="470px" align="center"><font size="10">' .
            $empresa . '<br />' . $almacen . ' ' . $estado_desc . '<br />' . 'SUCURSAL: ' .
            $row->sucursal . '<br />PROVEEDOR: ' . $row->proveedor . '<br />MOVIMIENTO: ' .
            strtoupper($row->movimiento) . '</font></td>
    ...
    function reporte_header_inv_ini()
    {
        ...
        $tabla = '<table cellpadding="5">
            <tr>
                ...
                <td rowspan="2" width="470px" align="center"><font size="10">' .
            EMPRESA . '<br />' . ALMACEN . ' ' . ESTADO_DESC . '<br />' .
            'REPORTE DE INVENTARIO INICIAL</font></td>
    ...
    function reporte_header_inv($tipo = null, $subtipo = null)
    {
        ...
        $tabla = '<table cellpadding="5">
            <tr>
                ...
                <td rowspan="2" width="470px" align="center"><font size="10">' .
            EMPRESA . '<br />' . ALMACEN . ' ' . ESTADO_DESC . '<br />' .
            'REPORTE DE INVENTARIO ACTUAL' . $a . '</font></td>
                <td width="65px">FECHA: </td>
                <td width="65px" align="right">' . date('Y-m-d') . '</td>
            </tr>
            <tr>
    ...
    function reporte_header_ajuste($id_ref)
    {
        ...
        $tabla = '<table cellpadding="5">
            <tr>
                ...
                <td rowspan="2" width="470px" align="center"><font size="10">' .
            EMPRESA . '<br />' . ALMACEN . ' ' . ESTADO_DESC . '<br />' .
            'REPORTE DE AJUSTE DE INVENTARIO A DETALLE</font></td>
                <td width="65px">REF.: </td>
    ...
    function reporte_detalle($id)
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $tabla .= '<tr>
                ...
                <td width="70px" align="right">' . number_format($row->piezas, 0) .
                '</td>
                <td width="70px" align="left">' . $row->lote . ' - ' .
                formato_caducidad($row->caducidad) . '</td>
            </tr>
            ';
        ...
        <tfoot>
            ...
            <tr>
                <td colspan="5" width="720"  style="font-size: x-large;">Observaciones: ' .
            $row2->observaciones . '</td>
            </tr>
        </tfoot>
        </table>';
    ...
    function formato($id)
    {
        ...
        $alm_formato = '<br /><br /><br /><table cellspacing="0" cellpadding="4" border="1" width="720">
<tr align="center">
<td>Embarco:</td>
<td colspan="2"></td>
<td colspan="6" rowspan="5">Folio:<br /><h1>' . $row->id .
            '</h1><br />Fecha:<br /><h1>' . $row->cerrado . '</h1></td>
</tr>
<tr align="center">
<td>Operador:</td>
<td colspan="2"></td>
...
<tr align="center">
<td colspan="3">' . ALMACEN . ' ' . $this->session->userdata('estado_nombre') .
            '</td>
...
<tr align="center">
<td colspan="2">' . $row->sucursal . ' ' . $row->proveedor . '</td>
<td>' . $row->movimiento . '</td>
<td>1</td>
<td>' . number_format($row->piezas, 0) . '</td>
<td></td>
<td></td>
<td></td>
<td></td>
...
<td colspan="2">Sello CEDIS ' . $this->session->userdata('estado_nombre') .
            ':</td>
</tr>
<tr align="center">
...
<td colspan="2">Firma del Operador:</td>
<td colspan="5">Colonia o poblacion:</td>
<td colspan="2">Sello Recepci&oacute;n:</td>
</tr>
<tr align="center">
<td colspan="2"><br /><br /><br /></td>
<td colspan="5"></td>
...
<td colspan="9" bgcolor="#F3F3F3"></td>
    ...
    function reporte_detalle_inv($tipo = null, $subtipo = null)
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $tabla .= '<tr>
                <td width="70px">' . $row->clave . '</td>
                <td width="510px">' . $row->descripcion . '</td>
                <td width="70px" align="right">' . number_format($row->inv, 0) .
                '</td>
                <td width="70px" align="left">' . $row->lote . ' - ' .
                formato_caducidad($row->caducidad) . '</td>
            </tr>
    ...
    function reporte_detalle_ajuste($id_ref)
    {
        ...
        foreach ($query as $row) {
            ...
            $tabla .= '<tr>
                ...
                <td width="70px" align="right">' . number_format($row->nueva - $row->
                vieja, 0) . '</td>
                <td width="70px" align="right">' . number_format($row->vieja, 0) .
                '</td>
                <td width="70px" align="right">' . number_format($row->nueva, 0) .
                '</td>
                <td width="80px" align="left">' . $row->lote . ' - ' .
                formato_caducidad($row->caducidad) . '</td>
            </tr>
            ';
        }
    ...
    {
        $sql = "SELECT clave, descripcion, unidad, sum(canreq) as canreq, sum(cansur) as cansur, sum(cast(canreq as signed) - cast(cansur as signed)) as negados, inv 
FROM pedidos p
...
order by sum(cast(canreq as signed) - cast(cansur as signed)) desc;";
    ...
    function surtidos($perini, $perfin)
    {
        $sql = "SELECT clave, descripcion, unidad, sum(canreq) as canreq, sum(cansur) as cansur, sum(cast(canreq as signed) - cast(cansur as signed)) as negados, inv 
FROM pedidos p
left join detalle d on p.id = d.p_id
...
where p.estatus = 3 and date(f_embarque) between ? and ?
group by clave
order by sum(cast(cansur as signed)) desc;";
    ...
    {
        $sql = "SELECT p.id, s.sucursal, canreq, cansur, cast(canreq as signed) - cast(cansur as signed) as negados
FROM pedidos p
left join detalle d on p.id = d.p_id
