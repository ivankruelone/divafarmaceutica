...
group by clave;";
    ...
    function autocomplete($term){
        $this->db->select('id, ean as clave, SUBSTRING(descripcion, 1, 255) as descripcion, lc, activo', FALSE);
        $this->db->or_like('descripcion', $term);
        $this->db->or_like('clave', $term);
        $this->db->or_like('ean', $term);
        $this->db->or_like('susa', $term);
    ...
    function update_producto(){
        ...
        $data->susa             = $this->input->post('susa');
        $data->descripcion      = $this->input->post('descripcion');
        $data->unidad           = $this->input->post('unidad');
        $data->precio_max       = $this->input->post('precio_max');
        $data->precio_diva      = $this->input->post('precio_diva');
